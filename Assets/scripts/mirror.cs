...
public class mirror : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (onScreen)
        {
            ...
            if (doppleganger.transform.GetChild(1).transform.childCount > 0)// && target.transform.GetChild(1).transform.childCount <= 0 )
            {
                Debug.Log("remove item");
                Destroy(doppleganger.transform.GetChild(1).transform.GetChild(0).gameObject);

            }
            else if (doppleganger.transform.GetChild(1).transform.childCount <= 0)// && target.transform.GetChild(1).transform.childCount > 0 )
            {
                Debug.Log("add item");
                ...
                child.parent = doppleganger.transform;
